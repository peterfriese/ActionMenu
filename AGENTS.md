...
- The detent is **detent-independent** (no fixed point) and **scroll-invariant**: it uses the rows' position relative to the sheet root, `rowsBottomInSheet = (rowsBottom - sheetTop) + contentOffset.y`, which is invariant under both detent changes and List scrolls — the global measurements shift together and cancel out, and adding back the scroll offset recovers the rows' unscrolled position, so handle drags, detent cycling, and scrolling the menu's List can never corrupt `contentHeight`. The target is `rowsBottomInSheet + sideMargin - sheetTopChrome`, where `sheetTopChrome` (≈21pt) is the sheet's grabber/rounded-top area that the presentation renders above the `.height` value — a fixed presentation constant, not a device or list style. Measuring inside a named coordinate space anchored at the sheet root was tried but is unreliable: the `.listRowBackground` views do not resolve the SwiftUI named space correctly.
