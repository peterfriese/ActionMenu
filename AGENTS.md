...
- It measures the rows' geometry at runtime: each row gets a `GeometryReader` background (via `.listRowBackground`) that reports its global frame through a private `RowBoundsKey` `PreferenceKey`; `ActionMenu` aggregates the last row's bottom edge (`rowsBottom`) and the rows' leading margin (`sideMargin`).
- The detent is solved as a **fixed point** so the bottom row ends at the same distance from the sheet's bottom as its own side margins: `target = currentDetent + rowsBottom + sideMargin − windowHeight` converges to `rowsBottom == windowHeight − sideMargin`. The `.medium` fallback anchors the first iteration via `windowHeight / 2`; if the row measurement hasn't landed yet, the fallback estimate is `contentSize + contentInsets` (the old chrome-inclusive formula).
- The window height is read from `UIScreen.main.bounds.height` — a documented UIKit exception, since SwiftUI exposes no way to obtain the presentation window's bounds from inside a sheet.
